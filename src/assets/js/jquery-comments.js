...
(function (factory) {
  ...
  var Comments = {
    ...
    events: {
      ...
      "change .commenting-field .textarea":
        "increaseTextareaHeight textareaContentChanged",
      ...
      "click .commenting-field .attachments .attachment .delete":
        "preDeleteAttachment",
      'change .commenting-field .upload.enabled input[type="file"]':
        "fileInputChanged",
    ...
    showActiveContainer: function () {
      var activeNavigationEl = this.$el.find(
        ".navigation li[data-container-name].active"
      );
      var containerName = activeNavigationEl.data("container-name");
      var containerEl = this.$el.find(
    ...
    addComment: function (commentModel, commentList, prependComment) {
      ...
      // Case: reply
      if (commentModel.parent) {
        var directParentEl = commentList.find(
          '.comment[data-id="' + commentModel.parent + '"]'
        );
    ...
    removeComment: function (commentId) {
      ...
      var commentElements = this.$el.find(
        'li.comment[data-id="' + commentId + '"]'
      );
    ...
    preDeleteAttachment: function (ev) {
      var commentingField = $(ev.currentTarget)
        .parents(".commenting-field")
        .first();
    ...
    preSaveAttachments: function (files, commentingField) {
      ...
      if (files.length) {
        if (!commentingField)
          commentingField = this.$el.find(".commenting-field.main");
        var uploadButton = commentingField.find(".control-row .upload");
        var isReply = !commentingField.hasClass("main");
        var attachmentsContainer = commentingField.find(
          ".control-row .attachments"
        );
        ...
        this.options.validateAttachments(attachments, function (
          validatedAttachments
        ) {
          if (validatedAttachments.length) {
            // Create attachment tags
            $(validatedAttachments).each(function (index, attachment) {
              var attachmentTag = self.createAttachmentTagElement(
                attachment,
                true
              );
    ...
    updateToggleAllButton: function (parentEl) {
      ...
      if (this.options.maxRepliesVisible === 0) {
        ...
        var togglableReplies = childComments.slice(
          0,
          -this.options.maxRepliesVisible
        );
      }
      ...
      if (
        toggleAllButton.find("span.text").text() ==
        this.options.textFormatter(this.options.hideRepliesText)
      ) {
    ...
    sortAndReArrangeComments: function (sortKey) {
      ...
      var mainLevelComments = this.getComments().filter(function (
        commentModel
      ) {
      ...
      $(mainLevelComments).each(function (index, commentModel) {
        var commentEl = commentList.find(
          "> li.comment[data-id=" + commentModel.id + "]"
        );
        commentList.append(commentEl);
      });
    },
    ...
    showActiveSort: function () {
      var activeElements = this.$el.find(
        '.navigation li[data-sort-key="' + this.currentSortKey + '"]'
      );
      ...
      if (this.currentSortKey != "attachments") {
        ...
        var defaultDropdownEl = this.$el
          .find(".navigation ul.dropdown")
          .children()
          .first();
    ...
    preSavePastedAttachments: function (ev) {
      ...
      if (files && files.length == 1) {
        ...
        var commentingField;
        var parentCommentingField = $(ev.target)
          .parents(".commenting-field")
          .first();
    ...
    toggleSaveButton: function (commentingField) {
      ...
      if ((commentModel = this.commentsById[textarea.attr("data-comment")])) {
        ...
        if (this.options.enableAttachments) {
          var savedAttachmentIds = commentModel.attachments.map(function (
            attachment
          ) {
          ...
          attachmentsChanged = !this.areArraysEqual(
            savedAttachmentIds,
    ...
    createComment: function (commentJSON) {
      ...
      if (
        this.currentSortKey == "attachments" &&
        commentModel.hasAttachments()
      ) {
    ...
    replyButtonClicked: function (ev) {
      ...
      var replyField = outermostParent.find(
        ".child-comments > .commenting-field"
      );
    ...
    createHTML: function () {
      ...
      if (this.options.noCommentsIconURL.length) {
        noCommentsIcon.css(
          "background-image",
          'url("' + this.options.noCommentsIconURL + '")'
        );
      ...
      if (this.options.enableAttachments) {
        ...
        if (this.options.attachmentIconURL.length) {
          noAttachmentsIcon.css(
            "background-image",
            'url("' + this.options.attachmentIconURL + '")'
          );
        ...
        if (this.options.uploadIconURL.length) {
          uploadIcon.css(
            "background-image",
            'url("' + this.options.uploadIconURL + '")'
          );
    ...
    createCommentingFieldElement: function (
      ...
      if (existingCommentId) {
        profilePictureURL = this.commentsById[existingCommentId]
          .profilePictureURL;
      ...
      var profilePicture = this.createProfilePictureElement(
        profilePictureURL,
        userId
      );
      ...
      var textarea = $("<div/>", {
        class: "textarea",
        "data-placeholder": this.options.textFormatter(
          this.options.textareaPlaceholderText
        ),
      ...
      if (existingCommentId && this.isAllowedToDelete(existingCommentId)) {
        // Delete button
        var deleteButtonText = this.options.textFormatter(
          this.options.deleteText
        );
      ...
      if (this.options.enableAttachments) {
        ...
        if (isMain) {
          textareaWrapper.append(
            uploadButton.clone().addClass("inline-button")
          );
        }
      ...
      if (parentId) {
        ...
        if (parentModel.parent) {
          ...
          var replyToTag = this.createTagElement(
            replyToName,
            "reply-to",
            parentModel.creator,
            {
      ...
      if (this.options.enablePinging) {
          ...
          [
            {
              ...
              template: function (user) {
                ...
                var profilePictureEl = self.createProfilePictureElement(
        ...
        $.fn.textcomplete.Dropdown.prototype.render = function (zippedData) {
          ...
          var top =
            ...
            self.options.scrollContainer.scrollTop();
          this.$el.css("top", top);
        ...
        $.fn.textcomplete.ContentEditable.prototype._skipSearch = function (
          clickEvent
        ) {
    ...
    createSpinner: function (inline) {
      ...
      if (this.options.spinnerIconURL.length) {
        spinnerIcon.css(
          "background-image",
          'url("' + this.options.spinnerIconURL + '")'
        );
    ...
    createCommentElement: function (commentModel) {
      ...
      if (commentModel.createdByCurrentUser)
        commentEl.addClass("by-current-user");
    ...
    createCommentWrapperElement: function (commentModel) {
      ...
      // Profile picture
      var profilePicture = this.createProfilePictureElement(
        commentModel.profilePictureURL,
        commentModel.creator
      );
      ...
      // Edited timestamp
      if (
        commentModel.modified &&
        commentModel.modified != commentModel.created
      ) {
        var editedTime = this.options.timeFormatter(commentModel.modified);
        var edited = $("<time/>", {
          class: "edited",
          text:
            this.options.textFormatter(this.options.editedText) +
            " " +
            editedTime,
      ...
      if (this.options.upvoteIconURL.length) {
        upvoteIcon.css(
          "background-image",
          'url("' + this.options.upvoteIconURL + '")'
        );
      ...
      if (
        commentModel.createdByCurrentUser ||
        this.options.currentUserIsAdmin
      ) {
    ...
    createUpvoteElement: function (commentModel) {
      ...
      var upvoteEl = $("<button/>", {
        class:
          "action upvote" +
          (commentModel.userHasUpvoted ? " highlight-font" : ""),
      })
        .append(
          $("<span/>", {
    ...
    reRenderCommentActionBar: function (id) {
      ...
      commentElements.each(function (index, commentEl) {
        ...
        $(commentEl)
          .find(".actions")
          .first()
          .replaceWith(commentWrapper.find(".actions"));
      });
    },
    ...
    createCssDeclarations: function () {
      ...
      this.createCss(
        ".jquery-comments ul.navigation li.active:after {background: " +
      ...
      this.createCss(
        ".jquery-comments .highlight-background {background: " +
          this.options.highlightColor +
          " !important;",
        +"}"
      );
      ...
      // Font highlight
      this.createCss(
        ".jquery-comments .highlight-font {color: " +
          this.options.highlightColor +
          " !important;" +
          "}"
      );
    ...
    setToggleAllButtonText: function (toggleAllButton, toggle) {
      ...
      var showExpandingText = function () {
        ...
        var replyCount = toggleAllButton.siblings(".comment").not(".hidden")
          .length;
        text = text.replace("__replyCount__", replyCount);
        textContainer.text(text);
      };

      var hideRepliesText = this.options.textFormatter(
        this.options.hideRepliesText
      );
    ...
    adjustTextareaHeight: function (textarea, focus) {
      ...
      var rowCount =
        focus == true
          ? this.options.textareaRowsOnFocus
          : this.options.textareaRows;
      do {
        setRows(rowCount);
        rowCount++;
        var isAreaScrollable =
          textarea[0].scrollHeight > textarea.outerHeight();
        var maxRowsUsed =
          this.options.textareaMaxRows == false
            ? false
            : rowCount > this.options.textareaMaxRows;
    ...
    ensureElementStaysVisible: function (el) {
      ...
      var minScrollTop =
        el.position().top +
        el.outerHeight() -
        this.options.scrollContainer.outerHeight();
    ...
    highlightPings: function (commentModel, html) {
      ...
      if (html.indexOf("@") != -1) {
        ...
        $(Object.keys(commentModel.pings)).each(function (index, userId) {
          ...
          html = html.replace(
            new RegExp(pingText, "g"),
            __createTag(pingText, userId)
          );
        });
      }
      return html;
